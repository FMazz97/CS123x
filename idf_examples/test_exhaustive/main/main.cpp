...
extern "C" void app_main(void) {
    ...
    // ---------------------------------------------------------------------------
    // 4. EXHAUSTIVE CHANNEL TEST (All 4 valid values)
    // ---------------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------------
    // 6. BATCH CONFIGURATION TEST (set_config / get_config)
    // ---------------------------------------------------------------------------
    ...
    ESP_LOGI(TAG, "  get_config() snapshot -> ch: %d | gain: %d | rate: %d | int_ref: %d",
             snapshot.channel, snapshot.gain, snapshot.rate, snapshot.int_ref);
    ...
    bool t_cal_auto = adc.calibrate_temp(25.0f);
    ESP_LOGI(TAG, "  calibrate_temp(25.0C) -> %s", t_cal_auto ? "[OK]" : "[FAIL]");

    // Read calibration parameters
    CS123X_TempParams tcal = adc.get_temp_calibration();
    ESP_LOGI(TAG, "  get_temp_calibration() -> ref_temp_c_degrees: %.2f | ref_temp_raw: %" PRIu32,
             tcal.ref_temp_c_degrees, tcal.ref_temp_raw);

    // Read temperature
    float temp_val = adc.read_temp(3);
