...
class CS123x {
private:
  uint8_t _cs123xType;  ///< ADC chip model (CS123X_TYPE_...)
  uint8_t _sclk;        ///< Serial Clock / Power Down GPIO pin
  uint8_t _dout;        ///< Bidirectional Data Output / Input GPIO pin

  uint8_t _refOff;    ///< Internal reference state (0x00 = ON, 0x01 = OFF)
  uint8_t _rate;      ///< Active data rate setting
  uint8_t _gain;      ///< Active PGA gain setting
  uint8_t _baseGain;  ///< Baseline PGA gain setting for normal channels
  uint8_t _channel;   ///< Currently selected input channel
...
public:
   ...
   */
  CS123x(uint8_t cs123xType, uint8_t dout, uint8_t sclk,
         uint8_t gain = CS123X_GAIN_128,
         uint8_t rate = CS123X_RATE_10Hz,
         uint8_t channel = CS123X_CH_A,
         uint8_t intRef = CS123X_INT_REF_OFF);
   ...
   */
  bool setRef(uint8_t intRef, bool verify = true);
   ...
   * @return true if successful, false otherwise.
   */
  bool setRate(uint8_t rate, bool verify = true);
   ...
   */
  bool setCh(uint8_t channel, bool verify = true);
