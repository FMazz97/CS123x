...
CS123x::CS123x(uint8_t cs123xType, uint8_t dout, uint8_t sclk,
               uint8_t gain, uint8_t rate, uint8_t channel, uint8_t intRef) {
	_dout = dout;
	_sclk = sclk;
...
uint32_t CS123x::getTimeoutMs() const {
	...
	// 640Hz	|		6.25ms					|		20ms
	// 1280Hz	|		3.125ms					|		10ms
	static constexpr uint32_t timeouts[4] = { 350, 100, 20, 10 };
...
bool CS123x::setRef(uint8_t intRef, bool verify) {
	if (intRef > CS123X_INT_REF_OFF) return false;  // Reject values > 1

	uint8_t currentRef = _refOff;
	_refOff = intRef;
...
bool CS123x::setRate(uint8_t rate, bool verify) {
	if (rate > CS123X_RATE_1280Hz) return false;  // Reject values > 3

	uint8_t currentRate = _rate;
	_rate = rate;
...
bool CS123x::setGain(uint8_t gain, bool verify) {
	...
	uint8_t currentGain = _gain;

	_baseGain = gain;

	// Fix gain = 1 for if on-chip temperature sensor was enabled
	if (_channel == CS123X_CH_TEMP) _gain = CS123X_GAIN_1;
	else _gain = gain;
...
bool CS123x::setCh(uint8_t channel, bool verify) {
	...
	uint8_t currentChannel = _channel;  // Backup
	uint8_t currentGain = _gain;
...
bool CS123x::setTempCalibration(float refTempC) {
	uint8_t previousChannel = _channel;
